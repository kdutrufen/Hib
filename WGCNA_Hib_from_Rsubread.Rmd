...
```{r, fig.width=7, fig.height=7}
meanME = list()
for (i in 1:length(which.module)){meanME[[i]] = as.vector(rbind(mean(ME[[i]][HibDesign2$condition=="S01"]), mean(ME[[i]][HibDesign2$condition=="S02"]), mean(ME[[i]][HibDesign2$condition=="S03"]), mean(ME[[i]][HibDesign2$condition=="S04"]), mean(ME[[i]][HibDesign2$condition=="S05"]),  mean(ME[[i]][HibDesign2$condition=="S06"])))}

sdME = list()
for (i in 1:length(which.module)){sdME[[i]] = as.vector(rbind(sd(ME[[i]][HibDesign2$condition=="S01"]), sd(ME[[i]][HibDesign2$condition=="S02"]), sd(ME[[i]][HibDesign2$condition=="S03"]), sd(ME[[i]][HibDesign2$condition=="S04"]), sd(ME[[i]][HibDesign2$condition=="S05"]),  sd(ME[[i]][HibDesign2$condition=="S06"])))}

library(plotrix)
std.errorME = list()
for (i in 1:length(which.module)){std.errorME[[i]] = as.vector(rbind(std.error(ME[[i]][HibDesign2$condition=="S01"]), std.error(ME[[i]][HibDesign2$condition=="S02"]), std.error(ME[[i]][HibDesign2$condition=="S03"]), std.error(ME[[i]][HibDesign2$condition=="S04"]), std.error(ME[[i]][HibDesign2$condition=="S05"]),  std.error(ME[[i]][HibDesign2$condition=="S06"])))}
...
HibPP_Variables = lapply(1:4, function(i){as.data.frame(cbind(HibPP[[i]][,9], HibPP[[i]][,7], HibPP[[i]][,13], HibPP[[i]][,15], HibPP[[i]][,11], HibPP[[i]][,12], HibPP[[i]][,21], HibPP[[i]][,16]))})
HibPP_Variables = lapply(HibPP_Variables, setNames, nm = c("CO2", "O2", "Acetate", "PRP", "Biomass", "Glucose", "Glucose_consumption", "Molecular_mass"))
...
names(HibPP_Variables_Vector) = c("CO2", "O2", "Acetate", "PRP", "Biomass", "Glucose", "Glucose_consumption", "Molecular_mass")
