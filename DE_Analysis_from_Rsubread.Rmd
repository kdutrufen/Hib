---
title: "DE analysis from Rsubread"
...
```{r }
RsubreadCountData = read.csv("~/Dropbox/Dados_Alinhamento/Rsubread_featurecounts/Rsubread_Count_Table_Renamed_without_rRNA.csv", header=TRUE, sep = "\t", row.names = 1)
countData = RsubreadCountData
...
par(mar=c(6,6,3,3))
barplot(c(mean(colSums(countData)), colSums(countData)), names.arg=c("m?dia", colnames(countData)), col=c("yellow", col.cell), las=2, main="", xlab="", ylab="", ylim=c(0,max(colSums(countData))))
title(ylab="Tamanho da biblioteca", line=4, cex.lab=1.5)
...
plotMDS(tmm, col=colors[as.factor(HibDesign2[,1])], pch=points[as.factor(HibDesign2[,1])], method="bcv")
#legend("topright", legend=levels(HibDesign2[,1]), pch=points, col=colors , ncol=2,cex=0.7)
title("Amostras")
```
...
groups = as.factor(colnames(df))

allgenesvar = filter.anova(df = df, groups = groups)

allgenesvar1 = sort(allgenesvar, decreasing = FALSE)

TopVAr4WGCNA = allgenesvar1[1:12000]
TopVAr4WGCNA <-   rownames(countData)[which(rownames(countData) %in% rownames(top)[1:4000])]

newcountData = countData[names(TopVAr4WGCNA),]
```
...
for (i in 1:nSets){is.de[[i]] = decideTestsDGE(LRT[[i]] , p.value=0.05)}
```
...
# Differential expression analysis
```{r, message=FALSE}
HibDesign2 = data.frame(row.names = colnames( countData ), condition = HibDesign$condition, set2$W)
design = model.matrix(~0+group+set2$W, data = HibDesign2)
colnames(design) <- c(levels(group), "W")
```
...
for (i in 1:nSets){DEKegg[[i]] =kegga(TrueDE_dataframe[[i]][,1], FDR = 0.01, species.KEGG = "hiu")}
