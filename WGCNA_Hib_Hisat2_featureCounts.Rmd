...
empirical <-   rownames(filtered)[which(!(rownames(filtered) %in% rownames(top)[1:500]))]
```
...
par(mfrow=c(1,3))
plotRLE(as.matrix(filtered$counts), outline=FALSE, ylim=c(-4, 4), col=col.cell, las = 2)
title(xlab="Samples", line=4, cex.lab=1.2)
...
fviz_dend(sampleTree, k = 6, # Cut in four groups
          cex = 0.5, # label size
...
plot(sft$fitIndices[,1], -sign(sft$fitIndices[,3])*sft$fitIndices[,2], xlab="Soft Threshold (power)",ylab="Scale Free Topology Model Fit",type="n", main = paste("Scale independence"))
...
# Minimum Module Size
```{r}
minModuleSizetest = seq(from = 10, to = 50, by = 1)
...
```{r, fig.width=7, fig.height=7}
meanME = list()
for (i in 1:length(which.module)){meanME[[i]] = as.vector(rbind(mean(ME[[i]][HibDesign1$condition=="S01"]), mean(ME[[i]][HibDesign1$condition=="S02"]), mean(ME[[i]][HibDesign1$condition=="S03"]), mean(ME[[i]][HibDesign1$condition=="S04"]), mean(ME[[i]][HibDesign1$condition=="S05"]),  mean(ME[[i]][HibDesign1$condition=="S06"])))}

sdME = list()
for (i in 1:length(which.module)){sdME[[i]] = as.vector(rbind(sd(ME[[i]][HibDesign1$condition=="S01"]), sd(ME[[i]][HibDesign1$condition=="S02"]), sd(ME[[i]][HibDesign1$condition=="S03"]), sd(ME[[i]][HibDesign1$condition=="S04"]), sd(ME[[i]][HibDesign1$condition=="S05"]),  sd(ME[[i]][HibDesign1$condition=="S06"])))}

library(plotrix)
std.errorME = list()
for (i in 1:length(which.module)){std.errorME[[i]] = as.vector(rbind(std.error(ME[[i]][HibDesign1$condition=="S01"]), std.error(ME[[i]][HibDesign1$condition=="S02"]), std.error(ME[[i]][HibDesign1$condition=="S03"]), std.error(ME[[i]][HibDesign1$condition=="S04"]), std.error(ME[[i]][HibDesign1$condition=="S05"]),  std.error(ME[[i]][HibDesign1$condition=="S06"])))}
...
arrows(MEbarplot,meanME[[i]] - std.errorME[[i]], MEbarplot,
       meanME[[i]] + std.errorME[[i]], lwd = 1.5, angle = 90,
       code = 3, length = 0.05)
}
```
...
modules = modules_genes_Hib = list()
# for (i in 1:length(moduleblocks)){modules[[i]] = rownames(moduleblocks[[i]])}
modules = modules_genes_Hib = lapply(moduleblocks, print(rownames))
names(modules) = which.module
...
# KEGG Enrichment Analysis of a gene set. Given a vector of genes, this function will return the enrichment KEGG categories with FDR control.
kk = compareCluster(modules_genes_Hib, fun="enrichKEGG", organism = 'hiu', pvalueCutoff=0.05)
...
# Boxplots
```{r}
par(mfrow=c(1,1), mar=c(5.1,4.1,4.1,2.1)+2, cex.axis=1,cex.lab=1)
# interesting_genhes = c("ENSG00000151892", "ENSG00000134352", "ENSG00000174600")

# interesting_genhes = modules[[1]]
interesting_genhes = c("HIB_11790", "HIB_11640", "HIB_11800", "HIB_11650", "HIB_11850", "HIB_11860", "HIB_11710", "HIB_11870", "HIB_11720", "HIB_11730", "HIB_11810", "HIB_11660", "HIB_11820", "HIB_11670", "HIB_11680", "HIB_11830", "HIB_11840", "HIB_11690", "HIB_10030", "HIB_11840", "HIB_11690")

# interesting_genhes = c("HIB_11840", "HIB_11830", "HIB_11820", "HIB_11810", "H733_0475", "NODE_366_length_1546_cov_11610.5_g324_i0", "NODE_6936_length_110_cov_38768_g6866_i0", "NODE_7424_length_88_cov_880582_g7354_i0", "NODE_3781_length_176_cov_36.7355_g3711_i0", "NODE_5701_length_126_cov_25505.4_g5631_i0")


# interesting_genhes = c("HIB_11840", "HIB_11830", "HIB_11820", "HIB_11810")

df = cpm(countData)
# df = cpm(Elder_countData)
df = df[c(interesting_genhes),]

for (i in 1:nrow(df)){
boxplot(df[i,]~HibDesign1$condition, las=2, ylab ="", xlab ="", col=c("blue", "red", "green", "orange", "purple"), main=interesting_genhes[i])
mtext("Samples", side = 1, line = 5.5)
mtext("Normalized Gene Expression (cpm)", side = 2, line = 4)
}
```
...
HibPP_Variables = lapply(HibPP, function(df){ select(df, c(2,9,7,13,15,11,12,21,16)) %>%
    set_names(c("EFT", "CO2", "O2", "Acetate", "PRP", "Biomass", "Glucose", "Glucose_consumption", "Molecular_mass")) %>%
    mutate(CO2 = gsub("%", "", CO2), 
           O2 = gsub("%", "", O2), 
           CO2 = as.numeric(gsub(",", ".", CO2, fixed = TRUE)), 
           O2 = as.numeric(sub(",", ".", O2, fixed = TRUE)), 
...
HibPP_Variables = lapply(1:length(HibPP_Variables), function(i) HibPP_Variables[[i]] %>% mutate(Batch = rep(paste0("B0", i), nrow(HibPP_Variables[[i]]))) %>% mutate(Time = paste0(0:21, "h")) ) # Categorical values: Batch, Time

library(plyr)
HibPP_Variables = ldply(HibPP_Variables, data.frame)
HibPP_Variables[HibPP_Variables == -Inf] <- NA

HibPP_Variables = as_tibble(HibPP_Variables)
HibPP_Variables$Time = factor(HibPP_Variables$Time, levels = c("0h",  "1h", "2h", "3h", "4h", "5h", "6h", "7h", "8h", "9h", "10h", "11h", "12h", "13h", "14h", "15h", "16h", "17h", "18h", "19h", "20h", "21h"))
HibPP_Variables$Molecular_mass = as.numeric(HibPP_Variables$Molecular_mass)
...
HibPP_Variables_2_plot[[1]] = HibPP_Variables[,c(2:5,32:33) ]
HibPP_Variables_2_plot[[2]] = HibPP_Variables[,c(6:9,32:33) ]
HibPP_Variables_2_plot[[3]] = HibPP_Variables[,c(10:13,32:33) ]
HibPP_Variables_2_plot[[4]] = HibPP_Variables[,c(14:17,32:33) ]
HibPP_Variables_2_plot[[5]] = HibPP_Variables[,c(18:21,32:33) ]
HibPP_Variables_2_plot[[6]] = HibPP_Variables[,c(22:25,32:33) ]
HibPP_Variables_2_plot[[7]] = HibPP_Variables[,c(26:29,32:33) ]
HibPP_Variables_2_plot[[8]] = HibPP_Variables[,c(30:31,32:33) ]

HibPP_Variables_2_plot = lapply(HibPP_Variables_2_plot, melt)

lapply(HibPP_Variables_2_plot, function(x) ggplot(x, aes(x = Time, y = value)) + geom_boxplot() + geom_jitter(aes(colour = Batch)) + theme_bw() + facet_wrap(~variable, scales = "free", ncol = 1))

HibPP_Variables_Samples = lapply(1:4, function(i){HibPP_Variables[[i]][points,]})
HibPP_Variables_Vector = lapply(1:ncol(HibPP_Variables_Samples[[1]]), function(j) as.vector(sapply(1:4, function(i){cbind(HibPP_Variables_Samples[[i]][,j])})))

lapply(HibPP_Variables_Samples, function(df){  c(mean(df[,-1]), var(df[,-1]), std.error(df[,-1])) })

library(plyr)
library(plotrix)
Mean_HibPP_Variables = aaply(laply(HibPP_Variables, as.matrix), c(2, 3), mean, na.rm= TRUE)
SE_HibPP_Variables= aaply(laply(HibPP_Variables, as.matrix), c(2, 3), std.error, na.rm= TRUE)
...
for (i in 2:4){ lines(HibPP_Variables[[i]][,2], type="o", pch=20+i, lty = i, col=cores[i]) }
...
datTraits = data.frame("Biomass" = HibPP_Variables_Vector$Biomass, "Acetate" = HibPP_Variables_Vector$Acetate, "Glucose" = HibPP_Variables_Vector$Glucose, "Consumed Glucose" = HibPP_Variables_Vector$Glucose_consumption, "PRP" = HibPP_Variables_Vector$PRP, "CO2" = HibPP_Variables_Vector$CO2, "Massa Molecular" = HibPP_Variables_Vector$Molecular_mass)
...
labeledHeatmap(Matrix = moduleTraitCor, xLabels = xL, yLabels = names(MEs), ySymbols = names(MEs), colorLabels = FALSE, colors = blueWhiteRed(50), textMatrix = textMatrix, setStdMargins = FALSE, cex.text = 0.6, zlim = c(-1,1), main = paste("Kinetic parameters - Modules relationship"), xLabelsAngle = 90)
```
