...
plot_genes_boxplot = function(df, genes, design, levels, converter, convert_from, convert_to){
  ...
  map(df_list, function(x) ggplot(x, aes(x = Condition, y = as.numeric(CPM), color = Condition)) + geom_boxplot(alpha = .5) + geom_jitter(alpha = .5,width = .1) + theme_bw() + theme(axis.text.x = element_text(angle=90, hjust=1)) + ylab(label = "Read counts (cpm)") + facet_wrap(~Gene, scales = "free", ncol = 2, labeller = as_labeller(appender)) )
}
```
...
sumdata %>% ggplot(aes(x=Samples, y=Counts, fill=Samples)) + geom_bar(colour="black", stat="identity", fill = c("yellow", col.cell) ) + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + geom_hline(mapping = NULL, data = NULL, yintercept = 2e+6, na.rm = FALSE, show.legend = NA, colour="black")
```
...
group=factor(Design$condition)
design <- model.matrix(~0 + group)
colnames(design) <- levels(group)
rownames(design) = colnames(countData)
fit <- DGEList(counts=countData, lib.size = colSums(countData), group=group) %>% calcNormFactors(method="TMM") %>% estimateDisp(design , tagwise=TRUE, robust=TRUE) %>% glmFit(design)
lrt <- glmLRT(fit ,coef=2:6)
top <-   topTags(lrt, n=nrow(fit))$table
empirical <-   rownames(countData)[which(!(rownames(countData) %in% rownames(top)[1:1000]))]
```
...
set2 <- RUVg(as.matrix(fit$counts), empirical, k=1)
...
```{r}
col.cell <- c("purple","orange", "blue", "red", "green", "black", "cyan", "darkgoldenrod", "darkorchid", "darkslategray")[as.factor(Design$condition)]
...
# edgeR pipeline
library(edgeR)
fit <- DGEList(counts = countData, lib.size = colSums(countData), group=group) %>% calcNormFactors("TMM") %>% estimateDisp(design, robust=TRUE) %>% glmFit(design)
rownames(fit$counts) = rownames(countData)
...
edgeR_results = map(1:nSets, function(i) edgeR_results[[i]]$table) %>% map(rownames_to_column) %>% map(mutate, pi_value = abs(logFC)*(-1)*log10(FDR)) %>% map(dplyr::arrange, FDR) 
edgeR_results = edgeR_results %>% map(mutate, Product = as.character(RefSeq_gff.genes$product[match(rowname, as.character(RefSeq_gff.genes$OldLocusTag))]))
edgeR_topDE1 = edgeR_results %>%map(dplyr::filter, FDR<THRESHOLD)
edgeR_topDE2 = edgeR_topDE1 %>% map(filter, abs(logFC) >= LFC )
edgeR_DE_names = edgeR_topDE2 %>% map(dplyr::select, rowname) %>% map(pull)
names(edgeR_DE_names) = c("S01xS02", "S01xS03", "S01xS04", "S01xS05", "S01xS06", "S02xS03", "S02xS04", "S02xS05", "S02xS06", "S03xS04", "S03xS05", "S03xS06", "S04xS05", "S04xS06", "S05xS06")
edgeR_n_DEGs = map(edgeR_DE_names, length)

# DESeq2 pipeline
library(DESeq2)
design = model.matrix(~0+group, data = Design)
colnames(design) <- c(levels(group))
coldata <- data.frame(row.names=colnames(countData), Design)
dds = DESeqDataSetFromMatrix(countData = countData, colData = coldata , design= ~ condition) %>% estimateSizeFactors %>% DESeq
...
DESeq2_results = DESeqresults %>% map(as.data.frame) %>% map(rownames_to_column) %>% map(arrange, desc(padj) ) %>% map(mutate, pi_value = abs(log2FoldChange)*(-1)*log10(padj))
DESeq2_results = DESeq2_results %>% map(mutate, Product = as.character(RefSeq_gff.genes$product[match(rowname, as.character(RefSeq_gff.genes$OldLocusTag))]))
DESeq2_topDE1 = DESeq2_results %>% map(filter, padj < THRESHOLD)
DESeq2_topDE2 = DESeq2_topDE1 %>% map(filter, abs(log2FoldChange) >= LFC )
DESeq2_DE_names = DESeq2_topDE2 %>% map(dplyr::select, rowname) %>% map(pull)
names(DESeq2_DE_names) = c("S01xS02", "S01xS03", "S01xS04", "S01xS05", "S01xS06", "S02xS03", "S02xS04", "S02xS05", "S02xS06", "S03xS04", "S03xS05", "S03xS06", "S04xS05", "S04xS06", "S05xS06")
DESeq2_n_DEGs = map(DESeq2_DE_names, length)

# limma-Voom pipeline
library(limma)
voom_fit = DGEList(counts = countData, lib.size = colSums(countData), group=group) %>% calcNormFactors("TMM") %>% voom(design, plot=FALSE) %>% lmFit(design)
efit = map(1:nSets, function(i) contrasts.fit(voom_fit, contrasts=contr.matrix[,i] ) ) %>%  map(eBayes, robust=TRUE)
limma_voom_results = map(1:nSets, function(i) toptable(efit[[i]], adjust="fdr", lfc=0, p.value=1.01, number=Inf) ) %>% map(as.data.frame) %>% map(rownames_to_column) %>% map(mutate, pi_value = abs(logFC)*(-1)*log10(adj.P.Val)) %>% map(arrange, desc(adj.P.Val) )
...
names(limma_voom_DE_names) = c("S01xS02", "S01xS03", "S01xS04", "S01xS05", "S01xS06", "S02xS03", "S02xS04", "S02xS05", "S02xS06", "S03xS04", "S03xS05", "S03xS06", "S04xS05", "S04xS06", "S05xS06")
limma_voom_n_DEGs = map(limma_voom_DE_names, length)
```
...
edgeR_DEGs = edgeR_DE_names

Hib_KEGG_enrichment = edgeR_DEGs %>% compareCluster(fun="enrichKEGG", organism="hiu", pvalueCutoff=0.05)

Hib_KEGG_enrichment@compareClusterResult = Hib_KEGG_enrichment@compareClusterResult %>% filter(p.adjust<0.05) %>% filter(Count>2)

Hib_KEGG_enrichment_df = Hib_KEGG_enrichment %>% as.data.frame

Hib_KEGG_enrichment_df$geneID = Hib_KEGG_enrichment_df %>% order_genes("geneID")

mar=c(5.1,4.1,4.1,2.1)
plot(Hib_KEGG_enrichment, type="dot", font.size = 12, includeAll=TRUE, showCategory=100)
```

```{r pathview}
library(pathview)
data_2_pv = edgeR_topDE2[[6]]$logFC
names(data_2_pv) = edgeR_topDE2[[6]]$rowname

# KEGG Argininie biosynthesis
pv.out <- pathview(gene.data = data_2_pv, pathway.id = "00220", species = "hiu", out.suffix = "hiu00220_Hib_S03xS02", kegg.native = T)

# Purine metabolism
pv.out <- pathview(gene.data = data_2_pv, pathway.id = "00230", species = "hiu", out.suffix = "hiu00230_Hib_S03xS02", kegg.native = T)
```
